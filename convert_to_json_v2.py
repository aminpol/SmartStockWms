...
# Esto captura todo lo que está entre ( y ), manejando comas dentro de comillas
matches = re.findall(r"\((.*?)\)", mysql_dump)

materials = []
for match in matches:
    # Dividir por comas, pero ignorar comas dentro de comillas
    parts = []
    current = ""
    in_quotes = False
    for char in match:
    ...
    # Limpiar comillas
    clean_parts = [p.strip("'") if p != "NULL" else None for p in parts]
    
    if len(clean_parts) >= 3:
        materials.append({
            "id_code": clean_parts[0],
            "description": clean_parts[1],
            "unit": clean_parts[2],
            "type": clean_parts[3] if len(clean_parts) > 3 else None
        })
...
print(f"✓ JSON generado con {len(materials)} materiales")
