...
# Tu dump MySQL completo (solo la parte de INSERT de materiales)
...
for match in matches:
    postgres_inserts.append(f"({match})")

# Generar el script SQL completo
output = f"""-- Script completo de importación de materiales para PostgreSQL
-- Total de materiales: {len(postgres_inserts)}
...
# Guardar el archivo
with open('c:/Generador de QR - copia/import_all_materials_complete.sql', 'w', encoding='utf-8') as f:
    f.write(output)

print(f"✓ Script generado con {len(postgres_inserts)} materiales")
print("✓ Archivo: import_all_materials_complete.sql")
